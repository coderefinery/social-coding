...
3. By default you are no authorized to use the content of a repository when there is no license. And derivative work is also not possible by default. However, the authors may have forgotten to add a license so we suggest you to contact the authors (e.g. make an issue) and ask whether they are willing to add a license.
...
6. As mentionned previously, MIT and BSD are GPL-compatible: it means that you can modify the original code without "infecting" the resultant derivative code with the original license. Problems arise when you wish to use another software with a "viral" license (requires that any derivative work also be licensed under a similar compatible license) that is not GPL-compatible. In that case, it may not be possible to combine all software.  
...
8. The least attractive licenses for companies with proprietary software are licenses where you would need to keep an open license when creating derivative work. For instance LGPL. The most attractive licenses are permissive licenses where they can reuse, modify and relicense with no conditions. For instance Apache License.
```
